...
class EC2
  def initialize(driver, wait, ec2_config)
    @driver = driver
    @wait = wait
  ...
  def configure_ec2_instance
    ...
    instance_type.clear
    instance_type.send_keys(@ec2_config[:instance_type])
    sleep(2)
    ...
    payment_type = @wait.until { @driver.find_element(:id, @ec2_config[:payment]) }
    payment_type.click
